...
class MultiDiGraphBuilder:
    ...
    def _identify_stations(self) -> None:
        n_stations = 0
        ...
        for agent in self.env.agents:
            for waypoint in agent.waypoints:
                station_id = n_stations + 1
                station_pos = waypoint[0].position
                if station_pos not in self.station_lookup.keys():
                    self.station_nodes[f'S{station_id}'] = station_pos
                    self.station_lookup[station_pos] = f'S{station_id}'
                    n_stations += 1
